...
borough_color <- colorFactor("viridis", ny_spdf$borough) 
...
leaflet() %>%
  ...
  addPolygons(data = ny_spdf,
              ...
              smoothFactor = 0.3, 
              fillOpacity = 0.2,
              fillColor = ~borough_color(ny_spdf$borough),
    highlight = highlightOptions(color = "white", weight = 1, bringToFront = TRUE)) %>%
  addPolylines(data = ny_spdf,
               color = ~borough_color(ny_spdf$borough), 
               weight = 1, 
               smoothFactor = 0.5, 
               opacity = 1.0, )
```
...
```{r}
# try to incorporate subset graphs into Leaflet
# subset df_final for just one date:
df_oneday <- df_final[df_final$date == "2020-04-01",]

df_oneday %>%
  group_by(name) %>%
  summarise_at(.vars = names(.)[23:24],.funs = c(mean="mean"))
...
```{r}
# find top 10 poorest zipcodes
# find top 10 richest zipcodes

top_10_richest <- unique(as.vector(df_final[order(-df_final$median_household_income),][1:78, 2]))
top_10_poorest <- unique(as.vector(df_final[order(df_final$median_household_income),][1:10, 2]))
```

```{r}
top_10_richest_df <- df_final[df_final$zip %in% top_10_richest,]

# rename ZIP column to postalCode to enable merge
colnames(top_10_richest_df)[which(names(top_10_richest_df) == "zip")] <- "postalCode"
top_10_richest_df$postalCode <- as.character(top_10_richest_df$postalCode)

# recode DATE column as date data type
top_10_richest_df$date <- as.Date(top_10_richest_df$date, "%Y-%m-%d")
top_10_richest_spdf <- df_leaflet
top_10_richest_spdf <- subset(top_10_richest_spdf, postalCode == 10306)
```

```{r}
top_10_poorest_df <- df_final[df_final$zip %in% top_10_poorest,]

# rename ZIP column to postalCode to enable merge
colnames(top_10_poorest_df)[which(names(top_10_poorest_df) == "zip")] <- "postalCode"
top_10_poorest_df$postalCode <- as.character(top_10_poorest_df$postalCode)

# recode DATE column as date data type
top_10_poorest_df$date <- as.Date(top_10_poorest_df$date, "%Y-%m-%d")
top_10_poorest_spdf <- df_leaflet
top_10_poorest_spdf <- top_10_poorest_spdf[top_10_poorest_spdf@data$postalCode %in% top_10_poorest, ]
```
